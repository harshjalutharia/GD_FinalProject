...
Shader "Custom/Grass"
{
	Properties
	{
		...
		[Toggle(_CAMERA_DISTANCE_CLIPPING_ON)] _CAMERADISTANCECLIPPING("Camera Distance Clip ON/OFF", Float) = 1
		_MinCamDistance ("Minimum Distance Distance", float) = 3
        _MaxCamDistance ("Maximum Distance Distance", float) = 100
	}
	...
	SubShader
	{
		...
		#pragma shader_feature_local _CAMERA_DISTANCE_CLIPPING_ON
		...
		void surf( Input i , inout SurfaceOutputStandardCustom o )
		{
			...
			// Control the alpha cutoff
			// Normally, this would be used to calculate which parts of the texture to hide -this makes the "grass blade" part of the texture appear only.
			// This time, we'll be multiplying this value by the distance threshold
			float ALPHAWEIGHT = 1.0;
			#ifdef _CAMERA_DISTANCE_CLIPPING_ON
				float dist = distance(i.worldPos, _WorldSpaceCameraPos);
				if (dist > _MinCamDistance) ALPHAWEIGHT = 1.0 - clamp(inverseLerp(_MinCamDistance, _MaxCamDistance, dist), 0, 1);
			#endif
			float GENERALALPHA505 = tex2DNode2.a;
			float ALPHACUTOFF496 = ( 1.0 - step( GENERALALPHA505 , ( 1.0 - _LeavesThickness ) ) );
			clip( ALPHACUTOFF496 * ALPHAWEIGHT - _MaskClipValue );
		}
