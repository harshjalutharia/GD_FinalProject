...
public class Slideshow : MonoBehaviour
{
    ...
    [SerializeField] private List<Slide> m_slides;
    ...
    public void NextSlide() {
        int nextSlideIndex = m_currentIndex + 1;
        if (nextSlideIndex >= m_slides.Count) {
            Debug.Log("Cannot advance to next slide - reached last slide");
            return;
        }

        StartCoroutine(TransitionCanvas(m_slides[nextSlideIndex]))
    }
    public IEnmuerator NextSlideCoroutine() {

    }

    public IEnumerator TransitionSlide(Slide c, float targetAlpha, float duration) {
        float startTime = Time.time;
        float endTime = startTime + duration;
        float startBuffer = duration * (1f - Mathf.Abs(targetAlpha - c.alpha));
        float currentTime = Time.time + startBuffer;
        while(currentTime < endTime) {
            currentTime = Time.time + startBuffer;
            c.alpha = Mathf.Abs((1f-targetAlpha) - (currentTime - startTime)/duration);
            foreach(CanvasGroup cg in c.groups) SetCanvasGroupAlpha(cg, c.alpha);
            yield return null;
        }
    ...
    */
    ...
    public IEnumerator SetCanvas(string name, bool setTo) {
        ...
        yield return SetCanvas(canvases[name], setTo);
    }
    public IEnumerator SetCanvas(_Canvas c, bool setTo) {
        ...
        yield return StartCoroutine(c.transition);
    }
